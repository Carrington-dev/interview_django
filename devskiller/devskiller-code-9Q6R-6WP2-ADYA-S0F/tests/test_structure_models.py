...
class StructureTestSuite(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.MODULE = leave.models

    def test_class_exists_leaverequest(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"TGVhdmVSZXF1ZXN0").decode(),
            classes,
            msg=f"The class "
            f"`{base64.b64decode(b'TGVhdmVSZXF1ZXN0').decode()}` "
            f"is not found, but it was marked as required.",
        )

    def test_class_exists_leaverequeststatus(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"TGVhdmVSZXF1ZXN0U3RhdHVz").decode(),
            classes,
            msg=f"The class "
            f"`{base64.b64decode(b'TGVhdmVSZXF1ZXN0U3RhdHVz').decode()}` "
            f"is not found, but it was marked as required.",
        )

    def test_class_exists_user(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"VXNlcg==").decode(),
            classes,
            msg=f"The class "
            f"`{base64.b64decode(b'VXNlcg==').decode()}` "
            f"is not found, but it was marked as required.",
        )
